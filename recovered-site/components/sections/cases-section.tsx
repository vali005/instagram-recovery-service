    ...
    <section className="px-6 py-24 sm:px-8 lg:px-12">
      <div className="mx-auto max-w-7xl rounded-[2rem] border border-slate-200 bg-white/80 p-8 shadow-[0_20px_80px_rgba(3,7,18,0.06)] backdrop-blur-xl sm:p-10 lg:p-12">
        <div className="max-w-3xl">
          ...
          <h2 className="mt-3 text-3xl font-semibold text-slate-950 sm:text-4xl">Что важно сделать в первые минуты.</h2>
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-3">
          {cases.map((item, index) => {
            ...
            <motion.article
              ...
              className="rounded-[1.5rem] border border-slate-200 bg-slate-950 p-6 text-white"
            >
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#2F80ED]/15 text-[#8BC4FF]"><Icon size={20} /></div>
              <h3 className="mt-4 text-xl font-semibold">{item.title}</h3>
              <p className="mt-3 text-sm leading-7 text-slate-300">{item.description}</p>
              <div className="mt-6 rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-sm text-slate-200">{item.action}</div>
            </motion.article>
          )})}
        </div>
      </div>
